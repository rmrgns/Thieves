...
AsyncRecv::AsyncRecv(SOCKET sock, WSABUF buf, IOContext* ctx) noexcept : socket(sock), recvBuffer(buf), ioContext(ctx) {};
...
void AsyncRecv::await_suspend(std::coroutine_handle<> handle) {
	...
	if (WSARecv(socket, &recvBuffer, 1, nullptr, &flags, ioContext->GetOverLapped(), nullptr) == SOCKET_ERROR) {
