...
Vector3 CBox::CalculateSliding(CBox& playerBox, Vector3 playerVelocity)
{
	...
	collisionNormal.x = playerVelocity.x *2;
	collisionNormal.y = playerVelocity.y *2;
	collisionNormal.z = playerVelocity.z *2;

	// 충돌 법선 벡터를 정규화
	float magnitude = sqrt(collisionNormal.x * collisionNormal.x + collisionNormal.y * collisionNormal.y + collisionNormal.z * collisionNormal.z);
	if (magnitude > 0.0f) {
		collisionNormal.x /= magnitude;
		collisionNormal.y /= magnitude;
		collisionNormal.z /= magnitude;
	}
