...
#include "client/object/actor/core/actor.h"
	...
	void MessageEventManager::RegisterMessageEvent(shared_ptr<MessageEventInfo>&& message)
	{
		m_message_queue.emplace(std::move(message));
	}

	void MessageEventManager::RegisterMessageReceiver(UINT event_id, const shared_ptr<Actor>& actor)
	{
		m_message_receive_actors.emplace(event_id, actor);
	}

	void MessageEventManager::UnregisterMessageReceiver(UINT event_id, const shared_ptr<Actor>& actor)
	{
		for (auto iter = m_message_receive_actors.lower_bound(event_id);
			iter != m_message_receive_actors.upper_bound(event_id); ++iter)
		{
