...
class NetworkMoveObj;
    ...
    {
    public:
        ObjectInfoMessageEventInfo(UINT event_id, const shared_ptr<NetworkMoveObj>& other);
        std::mutex m_obj_lock;
    private:
        shared_ptr<NetworkMoveObj>m_network_object;
    public:
        const shared_ptr<NetworkMoveObj>& GetNetworkObj()const { return m_network_object; }
    };
