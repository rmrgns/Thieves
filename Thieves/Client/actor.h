...
public:
	virtual void ExecuteMessage(const shared_ptr<MessageEventInfo>& message) {}
	virtual void ExecuteMessageFromServer(const shared_ptr<MessageEventInfo>& message) {}
...
private:
	...
	vector<std::string> m_registered_input_events;
	vector<UINT> m_registered_message_events;
//	UPtr<ComponentManager> m_component_manager;
//	UPtr<ActorPhysicsManager> m_physics_manager;

	//Mat4 m_world_matrix;
	Vector3 m_previous_position;
	Vector3 m_position;
